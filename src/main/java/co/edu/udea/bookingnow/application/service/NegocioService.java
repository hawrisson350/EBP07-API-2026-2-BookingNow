...
public class NegocioService implements RegistrarNegocioUseCase, ObtenerMiNegocioUseCase, ListarNegociosUseCase {
    ...
    public Negocio registrar(Long proveedorId, RegistrarNegocioCommand c) {
        ...
        String foto = v.base64(c.fotoPrincipalBase64(), "fotoPrincipalBase64", "IMAGEN");
        ...
        if (c.galeria() != null) {
            ...
            for (int i = 0; i < Math.min(c.galeria().size(), 20); i++) {
                ...
                String base64 = v.base64(item.base64(), "galeria[" + i + "].base64", tipo);
                if (base64 == null) { v.error("galeria[" + i + "].base64", "Base64 obligatorio"); }
                galeria.add(new Multimedia(base64, tipo, i));
            }
        }
        v.terminar();
    ...
    public List<Negocio> obtenerMisNegocios(Long proveedorId) { return negocios.obtenerPorProveedor(proveedorId); }
